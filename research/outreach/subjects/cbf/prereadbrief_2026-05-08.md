# Pre-read brief — Chesapeake Bay Foundation interview

**For:** [Recipient name TBD] · [Title TBD] · Chesapeake Bay Foundation
**From:** Chris Winn · author, *Commons Bonds* (forthcoming book + Noema-bound essay)
**Sent ahead of:** [interview date — TBD]
**Length cap:** one page (~440 words). Designed to be readable in three minutes on a phone. (Pre-customization template runs longer; trimmed per recipient before sending.)

**Status:** TEMPLATE — recipient-flexible. Customize the §"Where your work informs Ch 3" framing per recipient register (see *Author/Claude internal note* at bottom for staff-specific calibration). Live Q0 attribution per `research/outreach/_protocols/interview-attribution-protocol_2026-05-06.md`.

---

## The framework, in one paragraph

*Commons Bonds* names a structural mechanism — *cost severance* — that separates extracted value from the costs of producing it, transferring those costs to people, ecosystems, and futures who didn't capture the upside. The framework's interest is structural rather than ideological: the same mechanism shows up across many economic systems, so the question isn't "which system" but "what makes accountability bind." Two contributions. **Cost severance** names the mechanism. **Residual commons value (RCV)** names what it costs — the worth permanently foreclosed when a non-renewable resource is drawn down, *or when a renewable resource is pushed past regenerative capacity*. The Chesapeake — striped bass, oysters, menhaden, blue crab — is the framework's flagship case for the renewable-past-regeneration regime: a fishery that recovered once, and the question of whether the prior recovery dynamic is still available given climate, habitat, and stressor-stack changes.

## Where your work informs Chapter 3

Chapter 3 is the book's load-bearing case for *renewable-past-regeneration* and for what the book calls *regulatory architecture lagging the science* — where the management unit (Bay caps from historic landings; coastwide ERPs in place of Bay-specific assessment) doesn't keep up with the biology the science is showing. CBF's voice across stripers, oysters, menhaden, and blue crab — and CBF's working-waterfront network from Port Isobel to Smith Island to the Northern Neck — is the institutional backbone of the chapter. Your seat within that work is what I'd most value learning from.

## What this conversation is for

The book's success criterion is **vocabulary adoption** — its terms (*cost severance*, *RCV*) becoming useful where accountability work actually happens. This conversation is primarily a vocabulary calibration: where does the framework's language describe what your work already names under different vocabulary, where does it overreach, and where does it miss something Bay-specific? Pushback on framework terms is more valuable than confirmation; "this language doesn't quite fit because..." is the highest-leverage thing you can give me — it's what the manuscript can revise before publication.

## Questions I'd most value your view on

1. **Renewable-past-regeneration on the Bay.** Across stripers, oysters, menhaden, and blue crab — where has the prior recovery dynamic clearly stopped being available, where is it still available, and what's the structural difference?
2. **Regulatory architecture vs. the biology.** Where has the management unit — Bay cap from historic landings, coastwide modeling, sanctuary boundaries, ASMFC deferrals — most clearly failed to match the biological unit the science is describing?
3. **Cost-bearing today.** The 2026 oyster paradox — population recovering, watermen suffering — is one articulation of a structural mismatch. Where else on the Bay does that pattern repeat?
4. **Generations.** When you talk about *future generations* of the fishery, what specifically has been lost or is at risk of being lost, that prior cycles didn't lose?
5. **Voices the chapter needs.** Beyond your own, whose voice — watermen, restoration crews, captains, retired institutional memory, allied scientists, partner organizations — would most strengthen the chapter?

## On attribution

I'd like to quote you in the book and in any derivative essays (a *Noema*-track essay is in development; an *Aeon*-track piece has a separate angle). Practitioners and institutional voices handle attribution differently — some prefer freely-quotable answers, some prefer paraphrase + attribution unless reviewed first, others ask for full transcript review before any quoting. **I'll ask about your preference as the first thing in our conversation, before any of the framework questions** — so we have clear ground rules from the start. Whatever works for you, I'll abide by it across the book and essays.

---

*~30 minutes is what I'd planned for. I'll defer to whatever fits your day.*

---

*Author / Claude internal note (NOT sent): live Q0 verbal phrasing in `research/outreach/_protocols/interview-attribution-protocol_2026-05-06.md`. The five priority questions above are written broadly enough to work across staff levels. **Customize per recipient register before sending:***

- ***Hilary Falk (President/CEO) — strategic / institutional register.*** Lead with cross-program / org-level framing in §"Where your work informs Ch 3" (institutional posture, federalist coordination, the watershed-scale 2040 horizon). Q5 broadens to "voices and partner orgs across the watershed." Q1 can lead with stripers + the federalism story.
- ***Alison Prost (SVP Programs) — cross-program register.*** Emphasize program coordination across MD/VA/PA + Federal Director + Hampton Roads. Q5 emphasizes which voices Prost coordinates routing for.
- ***Will Poston (Forage Campaign Manager) — policy-specialist register.*** Emphasize menhaden, ASMFC architecture, the VIMS three-year study, observer requirements, the Bay-cap-from-historic-landings critique. Q1/Q2 collapse onto menhaden specifically.
- ***Jackie Shannon (VA Oyster Restoration Manager) — operational on-water register.*** Emphasize the Mobile Oyster Restoration Center, oyster gardening, Lynnhaven, restoration vs. wild-harvest dynamics. Q1 emphasizes oyster restoration specifically; Q3 emphasizes restoration crews + partner aquaculturists in cost-bearing.
- ***Christy Everett (Hampton Roads Director) — community-engagement register.*** Emphasize Lower Bay / Norfolk / Lynnhaven; Hampton Roads as constitutive of how the Bay is known. Q5 emphasizes community-level voices Everett can route to.
- ***Tanner Council (COA Senior Manager) — coalition-building register.*** Emphasize the 110+ COA partner network; aquaculturist voices (Bevans, Rappahannock Oyster Co., Rogue); restoration-and-aquaculture-coexistence framing.
- ***Captain at Port Isobel (current incumbent — name TBD) — working-waterfront / educator register.*** Drop the analytical framework opening density; lead with the chapter's interest in Tangier-archipelago watermen voice + the educator role. Q1 collapses to "what you've watched change on the water." The framework one-paragraph still appears but trimmed; emphasize the listening register.
- ***Bill Goldsborough (retired) — historical-memory register.*** Emphasize the 1980s striped-bass collapse + ERP architecture + the "1980s-recovery-not-replicable" question Moore has surfaced. Q1 leads with cycle comparison; Q2 leads with ASMFC architecture across decades.
- ***David Sherfinski (VA Communications) / Val DiMarzio (MD Communications) — gateway register.*** These are unlikely subjects for a substantive interview; if a "tell us more about the project" call materializes, this template can be radically trimmed (~250 words; framework one-paragraph + Q5 only) and reframed as a project-orientation rather than a substantive interview. Live Q0 still applies.

*Customize §"Where your work informs Chapter 3" content per recipient — keep ~75-90 words. Customize the framework-paragraph closing sentence ("The Chesapeake — striped bass, oysters, menhaden, blue crab...") to emphasize the species the recipient is closest to.*
