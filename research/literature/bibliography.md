...
- **Falcon 9 — launch capacity + cost (independent-first sourcing).** *Backs:* LEO payload capacity **22,800 kg** and the per-kg LEO cost (TA §11.10 launch-cost table). *Sources:* **NASA** Safety & Mission Assurance, *Space Launch Report: SpaceX Falcon 9 Data Sheet* (sma.nasa.gov — third-party-compiled, NASA-hosted) for the 22,800 kg capacity; **NASA** NTRS, *The Recent Large Reduction in Space Launch Cost* (ntrs.nasa.gov/citations/20200001093) for the launch-cost-reduction context. The **list price ~$74M (2026; year-stamped, rises over time)** is set by SpaceX as seller and corroborated by public reporting + NASA/GAO contract-award values → **~$3,245/kg** LEO. Confidence: HIGH (capacity — independent NASA-hosted source); MEDIUM / year-stamped (list price — re-confirm at publication). Web-verified 2026-06-06/07. **NB:** the §11.10 Falcon Heavy (`$90M / 63.8t`) + Starship cells need the same independent-source + year-pin treatment (SOFT follow-up).
