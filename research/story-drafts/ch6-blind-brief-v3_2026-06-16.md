<!-- STATUS: PROPOSED v3 | 2026-06-16 | supersedes ch6-blind-brief-v2_2026-06-10.md | session: claude/pm-book-completion-260610-07b75f -->

# Chapter 6 :: "Three Ways of Counting" :: Structural Drafting Brief v3 (BLIND)

**Date:** 2026-06-16 (v3 addendum; v2 body below unchanged, dated 2026-06-10)
**For:** a whole-cloth drafting agent who has NOT read and will NOT read the current Chapter 6, the prior redo drafts, or the redo-compare judgment.
**Supersedes:** `research/story-drafts/ch6-blind-brief-v2_2026-06-10.md`. v3 = the full v2 body (below, UNCHANGED) PLUS the `## ADDENDUM v3 2026-06-16` section appended at the end. **Read the addendum FIRST, then the body. Where the addendum names a REQUIRED beat or a structural spec, it is binding, and the body's corresponding beat is to be built to satisfy it; the addendum does not graft anything onto the chapter after the fact, it tells you what the named beats must contain when you write them from scratch.** The v2 body itself supersedes `research/story-drafts/ch6-blind-brief_2026-06-06.md` in full (including its 2026-06-09 M3 override banner, which is folded into the body here).
...
## ADDENDUM 2026-06-18 :: cross-session coordination (for the WHOLE-CLOTH rebuild) :: DO NOT skip

This brief now drives a fresh whole-cloth Ch 6 rebuild. Two parallel sessions ratified material that MUST land. Because this is a whole-cloth redraft, the line anchors below will NOT match the new draft: place every item BY CONTENT, at the concept it attaches to.

### (1) Ch 6 POSITIONING REQUIREMENT + GUARD (from the "neither addressed" L323-fix session :: author-ratified; commits f1fd6801 + 5cd59a12 + 1034f3ac on main)
GUARD (hard): NEVER ship "into a setting neither addressed" or any universal-negative claiming the drawn-down extraction setting was unaddressed. It is FALSE and self-contradictory: Hartwick 1977 / Dasgupta & Heal / genuine-savings priced it; Ostrom's CPR covers depletable stock (groundwater); the book itself credits Hartwick (Ch4, Ch5, Ch6).
LAND the corrected positioning: the contribution is the INSTRUMENT FORM + LEVEL, not the setting/idea. Concede the forebears; no fresh universal-negative. On the correct axes:
- Pigou: reserve against the FULL residual commons value (substitutability-weighted foreclosure + externality tail) vs Pigou's single marginal externality at time-of-damage.
- Ostrom: POWER/INCENTIVE ASYMMETRY. Prices the commons where coequality fails, the cost-bearer facing a counterparty with both the power AND the incentive to draw the commons down against them (absent/diffuse OR present-but-dominated). Do NOT use "drawn-down vs managed-in-place" (false; groundwater is both). Do NOT use "permanent foreclosure" as a scope gate (foreclosure is a VARIABLE RCV component; CS approximately 0 for sustainable harvest). Bridge to the Ch5 rent-seeking / Public Choice thread (the author considers this the strongest honest Ostrom axis).
- Hartwick/Dasgupta: MICRO per-extraction-event reserve-against bond vs MACRO savings-rule (Hartwick) / national inclusive-wealth accounting (Dasgupta). Name both as forebears the framework goes past at the per-event level.
MODEL TEXT: lift phrasing from the canonical em-dash-free contribution paragraph now on origin/main in Chapter__6_ThreeWaysofCounting.md (the L323 region). Mirrors book-proposal 00_overview.md.

### (2) H1-H5 matrix insertions targeting Ch 6 (drafted prose in tools/workstream-handoffs/matrix-h1-h5-ratified-insertions-handoff_2026-06-18.md :: carry by content)
- Insertion A (Broome co-grounding) :: first-name Broome in Ch 6 (full treatment; Ch 5 only forward-points).
- Insertion I (Solow weak-vs-strong sustainability + Dasgupta inclusive-wealth) :: at the four-properties / critic's-objection material.
- Insertion J (Dasgupta 2021 citation finish) :: LIGHT touch; COORDINATE with the L323 positioning text above (same sentence region).
All prose blocks are em-dash-free, substrate-safe. Keep consistent with proposal 00_overview, the Ch 5 rent-seeking thread, and the Ch 6 asymmetry material.
