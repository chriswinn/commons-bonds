# Chapter 6 — Internal Scaffolding (Drafting Direction)

**Layer:** internal-scaffolding per Working Principle #10 (Two-layer content origination discipline). The corresponding publisher-facing Draft is `Chapter__6_ThreeWaysofCounting__Draft.html` (HTML; single-line Google-Docs-export form pending Insight #21 dedicated rewrite session).

**Frontmatter added 2026-04-30** by Insight #37 separation pass (Thread β.1) — Ch 6 HTML Draft has no extractable scaffolding annotations under current single-line export format. Full WP#8/WP#10 discipline application at HTML scale is queued for the Insight #21 dedicated session (Tech Appendix v2.0.0 rebuild + Ch 6 HTML full rewrite in pretty-printed semantic HTML form).
...
| § Convergence result + IPG table (line ~103) | CURRENT | **VERIFIED 2026-05-08:** Ch 6 HTML draft (line 170 + line 433) DOES still include Exxon Valdez (1,200-1,900× IPG) as a backtest case alongside McDowell + Deepwater + Libby. This is intentional — Ch 5 dropped Exxon Valdez from its 6-case chapter scope, but Ch 6 uses it as a high-variance backtest case to span the IPG range (5× → 1,900×) for the convergence demonstration. Different chapters, different functions. Earlier 2026-05-08 audit incorrectly flagged this as PARTIALLY REGRESSED; corrected now. |
...
| § Counterargument 4 — "model is unfalsifiable" (line ~229) | CURRENT | Yeager-and-the-sound-barrier framing still canonical; **VERIFIED 2026-05-08:** §6.6 Passage H (CIT-vs-empirical falsifiability distinction) IS integrated at Ch 6 HTML lines 513 + 520 ("Two distinct falsifiability operations are at work... thought-experiment filter... empirical falsifiability operates one layer above it"). Integration landed at Insight #21 closure 2026-05-04. Earlier 2026-05-08 audit grep for the internal label "CIT-vs-empirical" returned 0 hits because the label is scaffolding-only; the actual prose content uses different phrasings ("thought-experiment filter" / "two distinct falsifiability" / "Popperian" — all present). Earlier flag corrected. |
...
8. **The Four Gates** *(new, Task 9; UPDATED 2026-04-24 Phase A3 sweep)* — paragraph-length summary of **CIT (Commons Inversion Test, formerly AIT) + Units Consistency (formerly Dimensional Consistency) + Boundedness + Independence** gates + one paragraph referencing Technical Appendix §L (gate formal statements) and §M (sum-of-costs generalization); ~500 words; Register 2. CIT introduction includes two sub-forms (Commons-Absence Inversion + Commons-Consumption Inversion) per Open Insight #16. Naming asymmetry (CIT is a TEST; other three are CONDITIONS) is structurally load-bearing per Four Gates cluster pass.
9. **The Contribution** *(new, Task 8; UPDATED 2026-04-24 Phase A3 sweep)* — ~1,050 words; FOUR distinctions from established economics traditions: substitutability-weighting + integrated architecture + **CIT method (formerly AIT)** + commons-governance lineage extension (Ostrom 1990; Hess & Ostrom 2007) + Hotelling Identity bridge to standard resource economics (Hotelling 1931); worked comparative examples for McDowell coal, Deepwater Horizon, Libby; Register 2, confident tone per rigor pass §11.4.
