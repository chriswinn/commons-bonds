...
**Methodology note (empirical validation of framework):** This case is the framework's strongest empirical validation. The diagnostic the framework makes (extraction operating across multiple commons, severing cost from value capture) was **already empirically validated by the public litigation record** before the framework was built. Sackler $11B extraction = framework's predicted value capture; $21B settlement = framework's predicted partial Accountability Bond (B); the $11B + $21B is still vastly less than RCV (deaths × statistical-life-value + community-recovery costs + multi-generational trauma). The framework's prediction matches what the courts found. Strong M2 + M6 + M11 case.
