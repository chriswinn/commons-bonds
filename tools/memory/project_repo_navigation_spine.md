...
**Update 2026-06-10 (structural-debt closeout):** `core/` is RETIRED — six top-level dirs remain (manuscript, publishing, research, tools, alignment, archive). The TA lives at `manuscript/technical-appendix/` (the book is whole under manuscript/); bibliography master + terms_index + glossary sources live at `tools/back-matter/sources/`; methodology at `alignment/methodology/`. The TA carries a generated §0 Notation section (regenerate via `build.py gen-ta-notation` after registry Part 7 changes).
