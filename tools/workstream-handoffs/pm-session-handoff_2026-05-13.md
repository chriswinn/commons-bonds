...
| **6** | **PROPOSED + 2 AMENDMENTS** `f117831` (B.2 + MUST-FIX-2 ratified + canonical-reading correction) + `a2f39ca` (Amendment B authoritative-source verification: 2 SHOULD-FIX + 5 MEDIUM date-refresh/disclosure items). **Darity-synthesis-incorporation gap pending**: MI-3 + SI-1 sharpening + SI-2 + C-3 + SI-3 disposition — see Ch 5 Pass 1 doc §B.7 for the cross-session flag | pending | pending | partial — see Ch 6 doc | **Fire Ch 6 Pass 1 Amendment (Darity-synthesis incorporation)** → Phase C-α single integrated commit (existing + new Darity items) → fire Pass 2. Required before Sandy send packet ships. |
