...
| When | Action |
|---|---|
| **Wed May 13** | **Darity interview COMPLETE.** Synthesis commit `3e39061` surfaced MI-1, MI-2, MI-3, SI-1, SI-2, SI-3, C-1, C-2, C-3. **APPLIED:** MI-1 + MI-2 into Ch 5 + TA. **SI-1 APPLIED:** confirmed 2026-05-14 in Ch 6 line 124 (introduced commit `6a5ee42` as part of Ch 6 Phase C-α post-Darity Pass-1 spot-fixes) — sharpened paraphrase of Sandy's profitability-vs-harms asymmetry framing; shipped to Sandy in the 2026-05-14 packet. **PENDING for Ch 6:** MI-3 (Ostrom homogeneity) + SI-2 (longevity gap as legacy-effect pricing) + C-3 (stratification economics tie). **PENDING for Ch 6 or Ch 10:** SI-3 (Sen). **Cross-thread:** C-1 (Fogel-Engerman two-volume model → #14 comp-titles), C-2 (Du Bois → bibliography). |
...
| **5** | RATIFIED + Phase C-α applied (`da26bdc`) | RATIFIED + Phase C-β + C-β-followup applied (`d68d92e` + `7cbb9c1` + `d2ee178`) | **PROPOSED 2026-05-14** — see [Pass 3 doc](../rigor-passes/commons_bonds_ch5_stage_3_pass_3_audience_load_2026-05-14_PROPOSED.md); awaits ratification + Phase C-δ | C-α + C-β + C-β-followup applied | **Ratify Pass 3 → apply Phase C-δ spot-fixes.** Packet shipped to Sandy 2026-05-14 was post-Phase-C-β-followup state (pre-Pass-3-ratification). |
| **6** | RATIFIED + Phase C-α applied (`6a5ee42`) + Pass 1 Amendment E / math-check Phase C-β applied (`f6bb6ad`) | RATIFIED + Phase C-γ applied (`f081ceb`) + C-γ-followup (`ccd87f1`) + C-δ tighten (`bfafe47`) + cascade-followup convergence-table figures refresh (`e927e74`) | **PROPOSED 2026-05-14** — see [Pass 3 doc](../rigor-passes/commons_bonds_ch6_stage_3_pass_3_audience_load_2026-05-14_PROPOSED.md); awaits ratification + Phase C-ε (if any) | extensive — see commit chain | **Ratify Pass 3 → apply Phase C-ε spot-fixes (if any).** Packet shipped to Sandy 2026-05-14 was post-cascade-followup state (pre-Pass-3-ratification). **Darity-synthesis disposition update:** SI-1 (profitability-vs-harms asymmetry) **RESOLVED in Ch 6 line 124** (commit `6a5ee42`, Phase C-α). Q0 tier-S citation question logged in the cover-email reply-handling list. **Still pending for Ch 6 / Ch 10:** MI-3 (Ostrom homogeneity), SI-2 (longevity gap), C-3 (stratification econ), SI-3 (Sen, Ch 6 or Ch 10) — reconcile in next PM session by reading Pass 1 Amendment E + Pass 3 docs against this list. |
...
|---|---|---|---|---|
| **William ("Sandy") Darity Jr.** | Duke | **PACKET SENT 2026-05-14** | Wed May 13 14:30 ET interview + Thu May 14 packet send | Interview conducted Wed. Synthesis commit `3e39061` surfaced MI-1 / MI-2 / MI-3 / SI-1 / SI-2 / SI-3 / C-1 / C-2 / C-3. MI-1 + MI-2 incorporated into Ch 5 + TA via Approach B (`70dce3f` + downstream). **SI-1 RESOLVED 2026-05-14 in Ch 6 line 124** (commit `6a5ee42`, Phase C-α). Still pending for Ch 6 / Ch 10: MI-3, SI-2, C-3, SI-3. **Thu May 14 — packet (Ch 5 + Ch 6 + TA) SENT** with cover email at [packet-send-cover-email_2026-05-14.md](../../research/outreach/subjects/darity/packet-send-cover-email_2026-05-14.md). Author cleared Ch 6 Pass-3 + TA Pass-3 gate by traveling-day decision. **Open follow-ups:** (1) Pass 3 spot-fixes (Ch 5 Phase C-δ + Ch 6 Phase C-ε) await ratification — currently post-send polish, not gating; (2) reconcile remaining Darity-synthesis-pending items (MI-3 / SI-2 / C-3 / SI-3) against where they actually live in the manuscript; (3) Reply-handling protocol per cover-email §"What to do with Sandy's reply" — including Ch 5 line-220 attribution clearance into synthesis Section 2, **the new Q0 tier-S citation question for Ch 6 line 124 (SI-1 paraphrase) — author call on raise-proactively vs. wait-and-see vs. not-required**, and warm-intro template activation (`publishing/agents/post-darity-warm-intro-templates_2026-05-10.md` Variant A) if a candidate is named. |
