...
- **But the recomputed NUMBER is NOWHERE** — `Chapter__5_TheAccountabilityGap.md` says "one hundred and eight trillion" on **every** branch (main, ta-provenance, redraft, pipeline). The `Ch5-testdraft_2026-06-04.md` also still says $108T, with a `[CITATION GAP]` flag. **Author believes it was recomputed (<$54T, defensible) on a branch whose commit was lost.** Not recoverable from git as of this session.
- **NEXT:** either locate the lost computation (search uncommitted session logs / other machines) OR recompute now via a focused agent per the spec. This is a chapter-prose change → redraft campaign / author.
