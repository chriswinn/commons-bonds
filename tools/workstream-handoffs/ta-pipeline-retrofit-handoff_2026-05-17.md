...
| 4 render-integrity | YES — **DUAL-RENDER (comparison-mode); HIGHEST STAKES IN THE CORPUS** | TA is the **fourth and final** of the 4 chapters firing in parallel with the render-pipeline-standardization workstream — and the hardest comparison case (full math content; Plane-1 chars like 𝒞 U+1D49E; formula-integrity stakes; cross-reference anchors). Render via BOTH pipelines (laptop `build-derivatives-alt.sh` for HTML→PDF + .docx; mobile-per-standardization-workstream-§3.1). Verify: every formula renders correctly; Greek letters; ≈; minus-sign vs hyphen-minus in math; sub/superscript HTML markup; table cell-integrity in §11.x deep-calibration blocks. Capture xelatex stderr for any `.md`-derived TA build path. Capture per-chapter comparison artifact. Stage 4 verdict = **PROPOSED-pending-canonical-decision**. **TA's comparison is the cumulative-diagnosis tipping point** for the canonical-pipeline decision; if any pipeline can't handle TA, that's a load-bearing finding. Do NOT tune the pipeline mid-comparison. |
