...
> **✓ CANONICAL-PIPELINE QUESTION RESOLVED 2026-05-18 — Docker (laptop via Colima) + remote-container (Anthropic cloud) share one canonical toolchain installed by [`tools/scripts/install-render-toolchain.sh`](../scripts/install-render-toolchain.sh).** Per the render-toolchain-containerization workstream ([`render-toolchain-containerization-handoff_2026-05-18.md`](../workstream-handoffs/archive/render-toolchain-containerization-handoff_2026-05-18.md)), the canonical pipeline is apt-installed pandoc + xelatex + wkhtmltopdf + EB Garamond + DejaVu Serif on Ubuntu 24.04. The same installer is consumed by [`Dockerfile.render`](../scripts/Dockerfile.render) (laptop + CI) and by the [`.claude/settings.json`](../../.claude/settings.json) SessionStart hook / Anthropic-web-UI setup script (remote-container).
>
> The earlier framing of this section anticipated an Option-A (laptop canonical) vs Option-B (remote-container canonical) vs Option-C (dual-discipline) decision. Resolution is closest to **Option B with a containerization shim**: the apt-installed Ubuntu pipeline is canonical; the laptop accesses it via Docker (Colima); the remote-container accesses it via the installer at session start. Both contexts produce byte-comparable output because they target the same Linux/amd64 + apt toolchain. The legacy laptop-native pipeline (`tools/scripts/build-derivatives.sh` with commercial Garamond + Chrome) remains in the repo as a separate context for cases where laptop-native rendering is wanted; it is NOT the canonical-pipeline output.
>
> Inside the canonical (Docker / remote-container) pipeline the canonical script is [`tools/scripts/build-derivatives-alt.sh`](../scripts/build-derivatives-alt.sh) (defaults to `MAIN_FONT="EB Garamond"`; auto-includes `fallback-header.tex`). Chrome is intentionally NOT installed — wkhtmltopdf is canonical for HTML→PDF per the Sandy packet (commit `e6ddf92`).
>
> **Reference renders (Sandy packet, canonical style baseline):** the with-citations Ch 5 + Ch 6 + TA artifacts at [`research/outreach/subjects/darity/`](../../research/outreach/subjects/darity/), delivered 2026-05-14 at commit `e6ddf92`. The 2026-05-18 render-toolchain-containerization workstream verified that a Docker render of those chapter sources at commit `9ffad4e` reproduces the docx byte-exact + the pdf within engine-version delta. Future Stage 4 audits target the Sandy with-citations artifacts as the canonical-style reference; the canonical Docker pipeline produces output of equivalent style and byte-comparable content.

Canonical conventions (per the ratified canonical pipeline above):
