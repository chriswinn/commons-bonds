...
**§14.8 Source manifest as a standard pre-pub artifact (refinement, ratified 2026-06-02).** Where the **venue's citation norm is inline / no bibliography** — the normal case for **magazine essays and op-eds** — the Stage 5 pre-publication review queue includes a **source manifest** as a standard component (§7.1 item 5): a sheet mapping every inline citation / factual claim → source → verification status. **The trigger is the venue's norm, not a fixed piece-type list.** Rationale: a magazine essay cites inline and ships **no bibliography** (appending one reads as a category error to a magazine editor — bibliographies belong to books and academic journals); the manifest is the internal fact-check support sheet that travels with the submission package and is what the venue's assigned fact-checker works from. **Match the venue's actual apparatus where it differs:** a venue that expects a formal reference list gets that instead; the **book** carries the bibliography (`tools/back-matter/sources/bibliography.md`). The manifest and the bibliography stay distinct so per-piece citation tracking does not duplicate or drift from the master bibliography (the 2026-06-02 Marx-entry sync was the empirical anchor for that drift risk). Location: `publishing/essays/<venue>/source-manifest.md`; template: `tools/drafting-templates/source-manifest-template.md`; reference instance: the Atlantic Ideas manifest.

**§14.9 Pass 3.1 traces headline / book-derived figures to SOURCE, not only to the brief (refinement, ratified 2026-06-02).** Pass 3.1's defined job is "audit the draft against the §7 canonical-facts inventory" — it checks **essay ↔ brief**, trusting the brief as ground truth. But the brief can itself carry an **ungrounded figure**, which then propagates through fact-check undetected. **Empirical anchor:** the Atlantic Ideas coal hook stated "$520–600/ton"; the $600 upper bound was carried from the original Locked hook and was never grounded in Ch 8 (whose breakdown tops at $524). Both the 2026-05-24 and the 2026-06-02 Pass 3.1 re-grounded only the *lower* bound and left the $600, because essay and brief matched. Refinement: for **headline and book-derived figures** (numbers traceable to a source chapter or a primary source), Pass 3.1 traces the figure to the **source**, not only to the brief. The **Gather phase** (§14 two-state citation, verify-against-source-before-it-enters-the-pool) is the structural fix for new essays; for legacy essays drafted from pre-Gather briefs, the trace-to-source check is the catch. (The Atlantic pilot drafted from a pre-Gather brief, so its $600 was baked in — the same root as its Mian/Sufi and longevity gaps.)

**§14.10 Memoir / personal-lived-material protocol (refinement, ratified 2026-06-02).** For pieces or passages drawn from the author's **personal memories and lived events**, the Gather + Plan phases do NOT generate or reconstruct lived detail. Instead the pipeline **presents what is already known/recorded** (the facts it has for that scene/subject) and asks the author to **add detail / confirm / correct / obfuscate** (a name or place, per the named-subject-consent discipline). Only author-supplied or author-confirmed lived detail enters the draft; the drafter never invents personal specifics from a brief. **Operationalizes as a Gather-phase branch:** lived-material rows in the source pool are flagged `author-confirm` and stay TENTATIVE (§ two-state citation) until the author confirms or supplies them. **Rationale:** memoir-register fresh generation is the *original* fact-drift failure mode — the v2.0 Noema test invented a "Tidewater-accent" etymology and reframed predawn Appalachian hunting drives as a Navy commute — so the no-invented-claims hard rule is most acute here, and audience-blind generation is the wrong default for lived material. **Anchor:** the deferred Noema rollout (2026-06-02). When Noema runs the new pipeline, it runs under THIS protocol, as the deliberate "can fresh beat the strongest iterated control?" test. Author articulation (2026-06-02): "start by presenting facts; for the ones that are personal memories and lived events, display what information you have, then see if I want to add detail, confirm, or obfuscate a name/place."
