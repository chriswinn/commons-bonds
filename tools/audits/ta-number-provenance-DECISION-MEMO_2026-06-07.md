...
| 3b Eco-floor NUMBER re-anchor (Path B) | **HANDED to Sessions C/D (IPG/M3 rework)** | Re-anchor the §11.6 Ecosystem M1 floor from the posited $50,000–200,000/acre to the documented **$12,000–15,500/acre** (Appalachian Voices 2021 cleanup-*cost* estimate — NOT the bonds-posted, so no M1/M2 conflation). Recompute: floor $7.5–30B → **~$1.8–2.3B**; per-ton ~$12–50 → **~$3–4/ton**; combined anchor $1,595–2,702 → **~$1,586–2,656/ton** (apply the same ~$9–46/ton subtraction to the at-scale + optimistic tiers). **Apply WITH the coal-$4.71 + IPG-ratio recompute** (Items 1/9b) so the combined-anchor change rides the same recompute and doesn't thrash. Optional — Path A already makes the section peer-defensible without it. |
