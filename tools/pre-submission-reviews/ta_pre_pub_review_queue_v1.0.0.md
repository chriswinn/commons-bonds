...
- **Stage 4 render-integrity audit verdict:** **RATIFIED 2026-05-20 — MEDIUM HOLD with all findings dispositioned** per [`tools/rigor-passes/render_pipeline_comparison_ta_2026-05-18.md`](../rigor-passes/render_pipeline_comparison_ta_2026-05-18.md). (Originally PROPOSED-pending-canonical-pipeline-decision 2026-05-18; canonical-pipeline RATIFIED 2026-05-19 via Docker containerization unblocked this verdict.) Triple-render summary:
  ...
  - **MEDIUM-severity finding F-RP-TA-02 (CORRECTED per Stage 4 §13 author visual review):** Plane-1 char `𝒞` renders correctly via Chrome HTML→PDF (laptop) AND via wkhtmltopdf Qt 5.15.13 (remote-container baseline); fails only via wkhtmltopdf Qt 4.8.7 (laptop build-derivatives-alt) per `cf24f57` doctrine §3.3 macOS .ttc-font-stack limitation. Rules out laptop wkhtmltopdf for HTML→PDF on macOS only; does NOT rule out remote-container wkhtmltopdf Qt 5.15.13.
...
- **§1.1 Definition 1.1 (Commons Resource) — notation legibility for cross-domain / layman readers (NEW, surfaced 2026-05-18 post-session author review).** Definition 1.1 reads: *"A resource unit R ∈ 𝒞 (where 𝒞 (script C) denotes the commons-territory set; distinguished typographically from C used as cost variable Cᵢ)..."*. Two readability frictions surfaced for a non-math reader: (a) the `∈` set-membership symbol (U+2208) is standard mathematical notation but unfamiliar to readers without prior math training; (b) the `(script C)` parenthetical reads as redundant when the `𝒞` glyph renders correctly (the parenthetical was likely written defensively for screen-reader-pronunciation cases + the wkhtmltopdf-tofu case where the glyph fails to render — see F-RP-TA-02). **Disposition (author-ratified 2026-05-18):** rendering as intended (not a render-failure; the laptop-Chrome pipeline renders `𝒞` + `∈` correctly per Stage 4 §3.3); flagged here as "hard for layman to read but rendering as intended." TA's primary target audience is PhD-economics / quantitative-reviewer (Sandy + downstream); the notation should be clear to that audience. **Trigger conditions for spot-fix:** (i) Sandy flags it as unclear during his 1–2 week TA + Ch 6 read, OR (ii) publisher / acquisitions editor flags it as not digestible enough for the target audiences. If neither trigger fires, hold as-is; the framework's notation conventions are correctly stated for the technical-reader-facing scope. If either trigger fires, rewrite (Pass 2 typography or Pass 3 pedagogical-scaffolding session per author preference at trigger time). Spot-fix candidate sketch (held pending trigger): rewrite Definition 1.1 to introduce the commons-territory set in plain English before attaching the symbol — *"A resource unit R is any draw from the commons-territory set, which the framework notates as 𝒞 (mathematical script-C, distinguished from the cost variable C used in Cᵢ). Set membership is written `R ∈ 𝒞`."*
