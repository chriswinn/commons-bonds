...
- **[`core/`](core/)** — canonical framework content: `glossary/` (HTML), `technical-appendix/` (math-heavy HTML), `terms/` (terms_index single-source-of-truth pipeline), `methodology/` (internal-scaffolding methodology artifacts per WP#10), `chapters/` (chapter audit), `case-studies/` (case-study audit). Live versions in this directory; superseded versions under per-domain `archive/`.
