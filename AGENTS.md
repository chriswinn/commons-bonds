...
| Framework-positioning disciplines | **v1.0.0 canonical 2026-05-06** — 10 disciplines (FPD-1 through FPD-10) governing how the framework gets positioned to audiences. Grounded in the project's success criterion (vocabulary adoption at scale per README) — interviews exist primarily to road-test framework vocabulary in the domains where it would be used. Surfaced 2026-05-05 → 2026-05-06 during Ch 3 fieldwork outreach pipeline + Darity interview prep. Disciplines: public-record screen, AI-disclosure first-contact-only, forwardability criterion, formality discipline, pre-read brief structure, two-artifact pair (pre-read + interview prep), attribution protocol with Q0 + four preference categories, scope-explicitness ("lead them to water" — both regimes named explicitly with example pairs), substantive background brief (subject's body of work + stances + existing domain vocabulary; drafted EARLY to inform the interview prep doc), credibility-first question framing with axis-matched structure (don't ask naively about settled territory; match question structure to subject's authority on the axis). | [`alignment/commons_bonds_framework_positioning_disciplines_v1.0.0.md`](alignment/commons_bonds_framework_positioning_disciplines_v1.0.0.md) |
