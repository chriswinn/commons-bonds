...
| Pipeline doctrine | **v1.0.0 + Amendment A (selective stage-firing, ratified 2026-05-18) + Amendment B (Pass 3.5 developmental-edit, ratified 2026-05-18) + Amendment C (interactive ratification protocol for prose-modifying passes, ratified 2026-05-19)** — six-stage architecture (Stage 0 → 1 [1a invariant + 1b structure + 1c coherence] → 2 → 3 [3.1 fact-check + 3.2 voice-polish + 3.3 acceptance + 3.4 robustness + **3.5 developmental-edit**] → 4 render-integrity → 5 sign-off bookend + pre-pub review queue) + continuous invariant-gate scans + two-class change-cascade routing (automatic-on-edit: Pass 3.1+3.2; explicit-gate: Pass 3.3+3.4+3.5+Stage 4+Stage 5) + single-pipeline content-type sub-protocols + cross-chapter workstream lifecycle + **interactive ratification protocol for prose-modifying passes (Amendment C; two-session workflow — discovery + combined ratification+application; each finding gets Options + Recommendation + Reasoning + author disposition before chapter source changes apply)**. Hybrid versioning: memory entry summary at `feedback_audience_aware_drafting_discipline.md` v3.1.0 (memory-update spec at `tools/memory-updates/`) + full canonical doctrine in repo. All 11 original decisions ratified 2026-05-17; Amendments A + B ratified 2026-05-18; Amendment C ratified 2026-05-19. Per-chapter retrofit workstreams + per-chapter developmental-edit workstream both in flight. | [`tools/pipeline-doctrine/commons_bonds_pipeline_doctrine_v1.0.0.md`](tools/pipeline-doctrine/commons_bonds_pipeline_doctrine_v1.0.0.md) · [`stage-1`](tools/pipeline-doctrine/commons_bonds_pipeline_doctrine_stage_1_v1.0.0.md) · [`stage-4`](tools/pipeline-doctrine/commons_bonds_pipeline_doctrine_stage_4_v1.0.0.md) · [`stage-5`](tools/pipeline-doctrine/commons_bonds_pipeline_doctrine_stage_5_v1.0.0.md) |
| Invariant-gate registries | **v1.0.0 PROPOSED 2026-05-17** — scaffolding + regressed-pattern YAML registries (per ratified decision #7 YAML format) consumed by `tools/scripts/check-corpus-invariants.sh` for pre-commit + stage-transition + CI scans. ~35 scaffolding patterns + ~20 regressed-pattern seed entries (F-V1 through F-V6 named LLM-tics from Ch 1 Pass 2 + Pass 2 inventory's connective-tissue clichés + render-failure patterns + gate-decision-banned templates). Severity-tiered (HIGH/MEDIUM/LOW) with per-file allowlist support. Retrofit workstreams populate to ~80-120 total entries. Pre-commit hook installer + GitHub Actions CI workflow shipped. | [`tools/quality-gates/`](tools/quality-gates/) · [`script`](tools/scripts/check-corpus-invariants.sh) · [`CI`](.github/workflows/corpus-invariants.yml) |
| Render toolchain | **RATIFIED 2026-05-18** — canonical apt-installed pandoc + xelatex + wkhtmltopdf + EB Garamond + DejaVu Serif on Ubuntu 24.04. One installer ([`install-render-toolchain.sh`](tools/scripts/install-render-toolchain.sh)) consumed by three contexts: Docker (laptop via Colima — see [`Dockerfile.render`](tools/scripts/Dockerfile.render)), remote-container (Anthropic cloud via [`.claude/settings.json`](.claude/settings.json) SessionStart hook or web-UI setup script), and CI ([`render-verify.yml`](.github/workflows/render-verify.yml)). Chrome intentionally NOT installed — wkhtmltopdf is canonical for HTML→PDF per Sandy packet (e6ddf92). Strict `--platform=linux/amd64` for byte-reproducibility. Resolves Stage 4 doctrine §3.3 OPEN canonical-pipeline question. | [`installer`](tools/scripts/install-render-toolchain.sh) · [`Dockerfile`](tools/scripts/Dockerfile.render) · [`README`](tools/scripts/README.md) · [`build-environment.yaml`](tools/quality-gates/render-baselines/build-environment.yaml) |
...
| Framework-positioning disciplines | **v1.0.0 canonical 2026-05-06** — 11 disciplines (FPD-1 through FPD-11) + canonical framework articulations + canonical Three Tiers / Live-call setup templates. Grounded in the project's success criterion (vocabulary adoption at scale per README). Disciplines: public-record screen, AI-disclosure first-contact-only, forwardability criterion, formality discipline, pre-read brief structure, two-artifact pair (pre-read + interview prep) with Three Tiers + Live-call setup canonical templates, attribution protocol with Q0 + four preference categories, scope-explicitness ("lead them to water"), substantive background brief (subject's body of work + stances + existing domain vocabulary), credibility-first question framing with axis-matched structure + fit/adoption/misuse failure-mode disambiguation, contingency planning for live interviews. Plus 9 canonical framework articulations (Hotelling, Ostrom, Polanyi, Mazzucato, **Raworth/Doughnut Economics**, Boyce, Marx/Harvey/Klein, reparations economics, "why bonds vs. funds/accounts/trusts/reserves") — oral-friendly drop-in language for interview lineage-extension pivots. Full "why bonds" rationale in vocabulary strategy v1.0.1 §6.5. **Raworth/Doughnut articulation added 2026-05-07** — captures complementarity-at-different-scales framing (Doughnut = economy-system goal-state; Commons Bonds = actor-transaction-level pricing instrument that operates within the goal-state, particularly for imported/upstream cost severance). Includes McDowell-vs-Amsterdam anchor pair (failure-vs-aspiration spectrum) and references the water-deal hypothetical worked example (`research/outreach/subjects/dagan/water-deal-hypothetical_2026-05-07.md`) as candidate Ch 9 entering-wedges material. **Plus canonical framework one-paragraph summary (added 2026-05-07)** — default version + theoretical/critical-engagement and applied-implementation variant patterns + optional historical-depth customization. Replaces "Soviet industrial planning" with "command economies" (politically less charged for general-readership audiences; analytically equivalent). Darity pre-read brief sent 2026-05-06 with original Soviet language pre-canonicalization, not retrofitted; Amsterdam pre-read template synced 2026-05-07. | [`alignment/commons_bonds_framework_positioning_disciplines_v1.0.0.md`](alignment/commons_bonds_framework_positioning_disciplines_v1.0.0.md) |
...
| Open insights tracker | **7 OPEN** as of 2026-05-17: #36 (Ch 1 + Ch 3 conversational drafting — Ch 1 NOW DRAFTED 2026-05-04 as "The Quiet Math"; Ch 3 still queued behind Chesapeake fieldwork outreach), #39 (pre-publication external review), #63 (Tier word-level conceptual collision; quick-fix applied, focused rigor pass queued), #64 (unpriced subsidies — raised; capture-only), #65 (six-mechanism cost-severance enumeration — under-review 2026-05-04; full framework cascade pending), #66 (universal computational claim — under-review 2026-05-04; Ch 1 plant placed, Ch 7 three-case parallel pending), #67 (multi-machine memory sync discipline candidate WP amendment — surfaced 2026-05-17 from memory-migration audit; codification deferred pending drift-incident data). Closed since v1.51.0: #21 (Ch 6 supplementary placement — folded into Phase 3 Tech Appendix rebuild). Next free insight number: **#68**. | [`alignment/commons_bonds_open_insights_v1.0.0.md`](alignment/commons_bonds_open_insights_v1.0.0.md) |
...
| Manuscript drafting state | **10 of 10 chapters drafted** as of 2026-05-09. Ch 1 ("The Quiet Math") drafted 2026-05-04 + title ratified per `commons_bonds_rigor_pass_2026-05-04_ch1_title_candidates_v2.0.0.md`. **Ch 3 ("The Waterman") drafted 2026-05-09** under the substitution-hypothesis operational approach (public-record + Chris's documented Hampton material; live Colden interview becomes upgrade opportunity, not precondition); 7,167 words; two passages carry `<!-- v1 from public record; revisit post-Colden-interview -->` flags. Ch 2, 4, 5, 7, 8, 9, 10 audit-clean (post Phase 1 framework audit). Ch 6 HTML canonical (Phase 3 rebuild queued). | [`manuscript/chapters/`](manuscript/chapters/) |
| Aeon essay pitch | **First-pass draft 2026-05-04** in author review at [`publishing/essays/aeon-mask-of-abundance/_archive/prior-versions/aeon-pitch-commons-bonds-winn.md`](publishing/essays/aeon-mask-of-abundance/_archive/prior-versions/aeon-pitch-commons-bonds-winn.md). Working title: "The Quiet Math" displaced from book-title-only; pitch placeholder "Pricing the Air"; title workshop continued next session. Mars/universality cut (closed-habitat thought experiment + six abundance-masking mechanisms + bad-actor / no-bad-actor symmetry). Pitch ~290 words; submission target `aeon.co/pitch` first-week-of-month window. **Pending upstream revision:** revisit McDowell case-walk + universality framing after Darity interview (week of May 10). | [`publishing/essays/aeon-mask-of-abundance/`](publishing/essays/aeon-mask-of-abundance/) |
| Noema essay | **Withdrawn 2026-05-01** per consent-rejection scope decision (ex-wife medical material foreclosed); rewrite-plan ratified same day at [`publishing/essays/noema-commons-bonds/_archive/planning/rewrite-plan_2026-05-01.md`](publishing/essays/noema-commons-bonds/_archive/planning/rewrite-plan_2026-05-01.md). Path B (book-first, essay paraphrases) chosen: revise once Ch 1 substrate fills. Personal-narrative cut (distinct from Aeon's universality cut). §V third-anchor case GATED on Colden interview outcome. | [`publishing/essays/noema-commons-bonds/`](publishing/essays/noema-commons-bonds/) |
| Outreach pipeline (week of 2026-05-05) | **12 cold-outreach drafts sent 2026-05-05** per `research/outreach/_pipeline/interview-outreach-drafts_2026-05-01.md`; **Ch 3 fieldwork outreach** sent 2026-05-05 09:00 ET per `research/outreach/_pipeline/interview-outreach-drafts_2026-05-04_ch3-fieldwork.md`. **ACCEPTED:** William Darity Jr. (Duke) — interview week of May 10; reply scheduled 2026-05-06 08:00 ET; pre-read brief drafted at [`research/outreach/subjects/darity/prereadbrief_2026-05-05.md`](research/outreach/subjects/darity/prereadbrief_2026-05-05.md). **DECLINED + WARM-INTRO OFFERED 2026-05-06:** Kate Raworth (declined 2026-05-05 via EA Beth Ingledew; Beth followed up 2026-05-06 offering an introduction to the Amsterdam DEAL implementation team — meaningful upgrade since Amsterdam City Doughnut is the framework's cleanest live Pattern 2 case). **PENDING REVIEW:** Mariana Mazzucato (holding response 2026-05-06 from Adam Albrecht, Office of Prof. Mazzucato — UCL IIPP). **VAL DiMARZIO RESPONSE (CBF, on behalf of Allison Colden):** scheduled 2026-05-06 08:00 ET; Colden pre-read brief drafted at [`research/outreach/subjects/colden/prereadbrief_2026-05-06.md`](research/outreach/subjects/colden/prereadbrief_2026-05-06.md). Other 8 cold-outreach recipients: no response yet. | [`research/outreach/`](research/outreach/) |
...
- **[`manuscript/`](manuscript/)** — book content:
  - `chapters/` — 10 drafts (Ch 1, 2, 3, 4, 5, 7, 8, 9, 10 in markdown + Ch 6 in HTML) + 10 guidance docs + dedication + author's note. All 10 chapters drafted as of 2026-05-09 (Ch 3 under substitution-hypothesis discipline). `chapters/archive/` holds Ch 6 supplementary drafts pending integration into Phase 3 Tech Appendix rebuild.
...
- `publishing/essays/noema-commons-bonds/` (the Noema submission package) carries the active essay; historical drafts + withdrawal note + original submission docs (`*.docx`, `*.pdf`) preserved in `_archive/` subdirs — do not modify those. Never touch `publishing/essays/berggruen-prize-2026/` essay text (competition requires AI-free content).
