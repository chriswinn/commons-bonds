---
artifact: $100 Barrel — Phenomenal World pitch email (proposal; PW submits by emailed proposal, NOT full-essay-in-body)
date: 2026-06-03
status: DRAFT — author reviews + sends; submission is author-controlled
to: editorial@phenomenalworld.org
essay: essay.md (4,201w, complete, RATIFIED-AWAITING-SUBMIT)
note: PW guidance (phenomenalworld.org/about): "Email editorial@phenomenalworld.org with a proposal." Lead with the proposal; offer/attach the completed essay rather than pasting it in the body.
AUTHOR-CONFIRM: the bio line + the AI-collaboration disclosure (both bracketed) are yours to finalize — I have NOT invented credentials.
---

**To:** editorial@phenomenalworld.org
**Subject:** Pitch — "The $100 Barrel": pricing the permanent foreclosure of a finite resource

Dear Phenomenal World editors,

I'd like to propose a long-form essay, "The $100 Barrel" (about 4,200 words, complete and ready to read), on a specific gap in how economics prices exhaustible resources.

The argument begins with a simple observation. When a barrel of oil sells for a hundred dollars, that price is struck by and for the present; it measures present scarcity at the margin. What it cannot capture is the worth of the barrel's permanent absence to everyone who comes after, because the parties that worth is owed to are not at the table, and most of them are not yet born. I call that unpriced remainder the residual commons value, and the essay's central claim is structural: it sits entirely outside the discount-rate debate that has occupied climate economics for twenty years. Stern and Nordhaus, and Weitzman's work on fat-tailed catastrophe, are all pricing the damage flow from combustion, discounted back to the present. None of them prices the resource's absence as such, and that absence survives even Stern's low rate, because it is not a future flow to be discounted but a fact about the present.

The essay engages the literature by name rather than around it. It walks the lineage that has circled this gap without building on it (Marx on the surplus across class, Ostrom on the renewable commons, Daly on throughput, Hartwick on reinvested rents), and it draws the claim of future generations from Rawls, Sandel, and Parfit. It grounds the practical half in two oil economies: Norway, whose sovereign fund (now well over two trillion dollars, more than three hundred and ninety thousand per citizen) is Hartwick's rule made real and held inviolate across every government for a generation; and the Niger Delta, where the same physical act ran through an architecture that severed the costs onto people who could not bill anyone for them. The frame throughout is architectural rather than partisan: the principle that whoever creates a cost should bear it is one a conservative and a liberal can both reach, and Norway is the proof that they have.

It seems to me to sit squarely in Phenomenal World's lane: heterodox political economy, named institutions and named debates, the political economy of capital and the state, in the company of the pricing-and-extraction work you publish. It is also timely against the current resource-frontier moment (critical minerals, deep-sea mining), though it is built to outlast any single news peg.

[A line on me — author to finalize: I developed this pricing framework outside the academy over the past decade; it anchors a book-length project, *Commons Bonds*, now complete in draft. (Add any prior publications / professional background you'd like to include.)]

[AI-collaboration disclosure — author to finalize per your standard practice across the portfolio.]

The full essay is finished and I'd be glad to send it along; it's ready whenever you'd like to read it. Thank you for considering the proposal.

Best regards,
Chris Winn
[contact details]
